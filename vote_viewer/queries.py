...
votes AS (
    SELECT
        ...
        pl.score
    FROM bounds b
    JOIN post_like pl ON pl.person_id = b.user_id
    JOIN post p ON p.id = pl.post_id
    JOIN community c ON c.id = p.community_id
    WHERE b.content_type IN ('all', 'post')
      AND (b.score IS NULL OR pl.score = b.score)
    ...
    SELECT
        ...
        cl.score
    FROM bounds b
    JOIN comment_like cl ON cl.person_id = b.user_id
    JOIN post p ON p.id = cl.post_id
    JOIN community c ON c.id = p.community_id
    WHERE b.content_type IN ('all', 'comment')
      AND (b.score IS NULL OR cl.score = b.score)
      AND (b.community_id IS NULL OR c.id = b.community_id)
...
authored_comment_counts AS MATERIALIZED (
    ...
    FROM post parent_post
    JOIN comment authored_comment
      ON authored_comment.post_id = parent_post.id
